...
export const send_launch_all_4_email = async (contact_email: string) => {
    const from = EMAIL_SENDERS.INFO
    const to = contact_email
    const subject = 'LAST CHANCE ⏰ for YOUR EARLY BIRD BONUS'
    // Always resolved next to this file - only ever run via tsx directly
    // (src/server/scripts/send_launch_all_4_email.ts), never through the esbuild-bundled server.
    const html_template_path = path.resolve(__dirname, 'launch_all_4.html')

    const unsubscribe_payload = redirect_payload_schema.parse({
        redirect_url: `${LINKS.INTERNAL.NEWSLETTER.UNSUBSCRIBE}?utm_source=email&utm_campaign=${EMAIL_TEMPLATES.LAUNCH_ALL_4}&utm_medium=unsubscribe_link`,
        email: contact_email,
        utm_params: {
            utm_source: 'email',
            utm_campaign: EMAIL_TEMPLATES.LAUNCH_ALL_4,
    ...
    const smtp2go = {
        headers: {
            'X-Category': EMAIL_TEMPLATES.LAUNCH_ALL_4,
        },
    }
