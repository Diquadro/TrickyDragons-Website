...
export const send_welcome_vip_1_email = async (contact_email: string, first_name?: string) => {
    ...
    const html_template_path = ENV.LOCAL
        ...
        : path.resolve(
              __dirname,
              '..',
              'shared/templates/emails/welcome_email_vip_1',
              'welcome_email_vip_1.html',
          )
    ...
    const unsubscribe_payload = redirect_payload_schema.parse({
        redirect_url: `${LINKS.INTERNAL.NEWSLETTER.UNSUBSCRIBE}?utm_source=email&utm_campaign=${EMAIL_TEMPLATES.WELCOME_VIP_1}&utm_medium=unsubscribe_link`,
        email: contact_email,
        utm_params: {
            utm_source: 'email',
            utm_campaign: EMAIL_TEMPLATES.WELCOME_VIP_1,
    ...
    const smtp2go = {
        headers: {
            'X-Category': EMAIL_TEMPLATES.WELCOME_VIP_1,
        },
    }
