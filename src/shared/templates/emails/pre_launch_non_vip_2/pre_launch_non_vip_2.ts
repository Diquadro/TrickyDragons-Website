...
export const send_pre_launch_non_vip_2_email = async (contact_email: string) => {
    ...
    const html_template_path = path.resolve(__dirname, 'pre_launch_non_vip_2.html')

    const reservation_payload = redirect_payload_schema.parse({
        redirect_url: `${LINKS.INTERNAL.RESERVATION.WELCOME}?email=${Base64_Url.encode(contact_email)}&utm_source=email&utm_campaign=${EMAIL_TEMPLATES.PRE_LAUNCH_NON_VIP_2}&utm_medium=reservation_link`,
        email: contact_email,
        utm_params: {
            utm_source: 'email',
            utm_campaign: EMAIL_TEMPLATES.PRE_LAUNCH_NON_VIP_2,
            utm_medium: 'reservation_link',
        },
    })

    const unsubscribe_payload = redirect_payload_schema.parse({
        redirect_url: `${LINKS.INTERNAL.NEWSLETTER.UNSUBSCRIBE}?utm_source=email&utm_campaign=${EMAIL_TEMPLATES.PRE_LAUNCH_NON_VIP_2}&utm_medium=unsubscribe_link`,
        email: contact_email,
        utm_params: {
            utm_source: 'email',
            utm_campaign: EMAIL_TEMPLATES.PRE_LAUNCH_NON_VIP_2,
    ...
    const smtp2go = {
        headers: {
            'X-Category': EMAIL_TEMPLATES.PRE_LAUNCH_NON_VIP_2,
        },
    }
