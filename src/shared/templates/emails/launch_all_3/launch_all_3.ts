...
export const send_launch_all_3_email = async (contact_email: string) => {
    ...
    const unsubscribe_payload = redirect_payload_schema.parse({
        redirect_url: `${LINKS.INTERNAL.NEWSLETTER.UNSUBSCRIBE}?utm_source=email&utm_campaign=${EMAIL_TEMPLATES.LAUNCH_ALL_3}&utm_medium=unsubscribe_link`,
        email: contact_email,
        utm_params: {
            utm_source: 'email',
            utm_campaign: EMAIL_TEMPLATES.LAUNCH_ALL_3,
    ...
    const smtp2go = {
        headers: {
            'X-Category': EMAIL_TEMPLATES.LAUNCH_ALL_3,
        },
    }
