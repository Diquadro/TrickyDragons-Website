...
/**
 * Welcome Email Non-VIP #1 Cron Job
 *
 * Sends first welcome email to newsletter subscribers who haven't purchased a reservation.
 *
 * Logic:
 * - Subscribed to newsletter
 * - NO reservation purchase (not in paid orders)
 * - Haven't received welcome email v1 or welcome_non_vip_1 yet
...
export const welcome_email_non_vip_1_cron = new CronJob(
    CRON_INTERVAL,
    async () => {
        console.log('🕒 Running welcome_email_non_vip_1 cron job...')

        try {
            await process_welcome_non_vip_1_emails()
        } catch (error) {
            console.error('❌ Error in welcome_email_non_vip_1 cron job:', error)
        }
    },
...
/**
 * Main processing function for welcome non-VIP #1 emails
 */
export async function process_welcome_non_vip_1_emails() {
    const eligible_contacts = await get_eligible_contacts()

    console.log(`📧 Found ${eligible_contacts.length} contacts eligible for welcome_non_vip_1 email`)
    ...
    for (const contact of eligible_contacts) {
        try {
            ...
            console.log(`✅ Sent welcome_non_vip_1 email to ${contact.email}`)

            // Small delay between emails
            await sleep(1000)
        } catch (error) {
            console.error(`❌ Failed to send welcome_non_vip_1 email to ${contact.email}:`, error)
        }
    }
}

/**
 * Get contacts eligible for welcome_non_vip_1 email
 *
 * Criteria:
 * - Subscribed to newsletter
 * - NO reservation purchase (not in paid orders)
 * - Haven't received welcome v1 or welcome_non_vip_1 yet
 * - Created at least EMAIL_DELAY ago
 */
async function get_eligible_contacts(): Promise<Contacts[]> {
    ...
    const contacts = await sql<Contacts[]>`
        ...
        WHERE 
            ...
            -- Must not have received welcome emails yet
            AND (
