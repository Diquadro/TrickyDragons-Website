...
import { send_welcome_vip_2_email } from '@shared/templates/emails/welcome_email_vip_2/welcome_email_vip_2'
...
/**
 * Welcome Email VIP #2 Cron Job
 *
 * Sends second welcome email to newsletter subscribers who have purchased a reservation,
 * after the first VIP email was delivered with the specified delay.
 *
 * Logic:
 * - Subscribed to newsletter
 * - HAS reservation purchase (exists in paid orders)
 * - Already sent welcome_vip_1
 * - Haven't sent welcome_vip_2 yet
 * - First VIP email was delivered at least [EMAIL_DELAY] ago
 *
 * Timing: Every 2 minutes (dev/local) or 10 minutes (production)
 */

// Development: 5 minutes delay, check every 2 minutes
// Production: 24 hours delay, check every 10 minutes
const EMAIL_DELAY = ENV.DEVELOPMENT ? 5 * 60 * 1000 : 24 * 60 * 60 * 1000 // 5min vs 24h
const CRON_INTERVAL = ENV.PRODUCTION ? '*/10 * * * *' : '*/2 * * * *' // 10min vs 2min

export const welcome_email_vip_2_cron = new CronJob(
    CRON_INTERVAL,
    async () => {
        console.log('🕒 Running welcome_email_vip_2 cron job...')

        try {
            await process_welcome_vip_2_emails()
        } catch (error) {
            console.error('❌ Error in welcome_email_vip_2 cron job:', error)
        }
    },
...
/**
 * Main processing function for welcome VIP #2 emails
 */
export async function process_welcome_vip_2_emails() {
    const eligible_contacts = await get_eligible_contacts()

    console.log(`📧 Found ${eligible_contacts.length} contacts eligible for welcome_vip_2 email`)
    ...
    for (const contact of eligible_contacts) {
        try {
            ...
            await send_welcome_vip_2_email(contact.email, first_name)
            await mark_email_as_sent(contact.uuid, EMAIL_TEMPLATES.WELCOME_VIP_2)

            console.log(`✅ Sent welcome_vip_2 email to ${contact.email} (${first_name})`)

            // Small delay between emails
            await sleep(1000)
        } catch (error) {
            console.error(`❌ Failed to send welcome_vip_2 email to ${contact.email}:`, error)
        }
    }
}

/**
 * Get contacts eligible for welcome_vip_2 email
 *
 * Criteria:
 * - Subscribed to newsletter
 * - HAS reservation purchase (exists in paid orders)
...
async function get_eligible_contacts(): Promise<Contacts[]> {
    ...
    const contacts = await sql<Contacts[]>`
        ...
        WHERE 
            ...
            -- Must have received welcome_vip_1 but not welcome_vip_2
            AND c.sent_emails @> ARRAY[${EMAIL_TEMPLATES.WELCOME_VIP_1}]::text[]
            AND (
                c.sent_emails IS NULL 
                OR NOT c.sent_emails @> ARRAY[${EMAIL_TEMPLATES.WELCOME_VIP_2}]::text[]
            )
            
            -- First VIP email must have been delivered at least EMAIL_DELAY ago
            AND EXISTS (
                SELECT 1 FROM actions a
                WHERE a.contact_uuid = c.uuid
                  AND a.action = ${API.EVENTS.ACTIONS.SMTP2GO_EMAIL_DELIVERED}
                  AND a.details->>'category' = ${EMAIL_TEMPLATES.WELCOME_VIP_1}
                  AND a.occurred_at < ${cutoff_date}
            )
