 ...
 */
async function create_contact_map(emails: string[]): Promise<Map<string, { uuid: ContactsUuid; email: string }>> {
...
async function process_event_for_recipient(
	...
	switch (event.event) {
		case 'processed':
			await handle_processed_event(event as Smtp2go_Processed_Event, contact.uuid, recipient_email, category)
			break
		case 'delivered':
			await handle_delivered_event(event as Smtp2go_Delivered_Event, contact.uuid, recipient_email, category)
			break
		case 'open':
		...
		case 'spam':
			...
			break
		case 'unsubscribe':
			await handle_unsubscribe_event(event as Smtp2go_Unsubscribe_Event, contact.uuid, recipient_email, category)
			break
		case 'resubscribe':
			await handle_resubscribe_event(event as Smtp2go_Resubscribe_Event, contact.uuid, recipient_email, category)
			break
		case 'reject':
...
function extractCustomHeaders(event: any): Record<string, any> {
	const knownFields = new Set([
		'event', 'email_id', 'id', 'rcpt', 'sender', 'from', 'from_address', 'from_name',
		'recipients', 'time', 'sendtime', 'subject', 'Message-Id', 'message-id', 'message',
		'context', 'host', 'auth', 'bounce', 'user-agent', 'read-secs', 'client',
		'client-device', 'client-os', 'geoip-continent', 'geoip-country', 'geoip-city',
		'srchost', 'url', 'X-Category', 'opened-at', 'clicked-at'
	])
...
async function handle_processed_event(event: Smtp2go_Processed_Event, contact_uuid: ContactsUuid, recipient_email: string, category: string): Promise<void> {
...
async function handle_delivered_event(event: Smtp2go_Delivered_Event, contact_uuid: ContactsUuid, recipient_email: string, category: string): Promise<void> {
	// Create action in database
	await create_action({
...
async function handle_open_event(event: Smtp2go_Open_Event, contact_uuid: ContactsUuid, recipient_email: string, category: string): Promise<void> {
	// Create action in database
	await create_action({
...
async function handle_click_event(event: Smtp2go_Click_Event, contact_uuid: ContactsUuid, recipient_email: string, category: string): Promise<void> {
	// Create action in database
	await create_action({
...
async function handle_bounce_event(event: Smtp2go_Bounce_Event, contact_uuid: ContactsUuid, recipient_email: string, category: string): Promise<void> {
...
async function handle_spam_event(event: Smtp2go_Spam_Event, contact_uuid: ContactsUuid, recipient_email: string, category: string): Promise<void> {
	...
	if (spam_count >= 2) {
		console.warn(`🚨 Second spam report detected for contact ${contact_uuid} - removing newsletter subscription`)
		await remove_newsletter_subscription(contact_uuid, 'spam')
	} else {
		console.warn(`🚨 First spam report for contact ${contact_uuid} - keeping subscription (count: ${spam_count})`)
	}
}

async function handle_unsubscribe_event(event: Smtp2go_Unsubscribe_Event, contact_uuid: ContactsUuid, recipient_email: string, category: string): Promise<void> {
	// Create action in database
	await create_action({
...
async function handle_resubscribe_event(event: Smtp2go_Resubscribe_Event, contact_uuid: ContactsUuid, recipient_email: string, category: string): Promise<void> {
	// For now, we log this but don't have a specific action constant for resubscribe
	// Could be added to constants if needed
}

async function handle_reject_event(event: Smtp2go_Reject_Event, contact_uuid: ContactsUuid, recipient_email: string, category: string): Promise<void> {
 ...
 */
async function remove_newsletter_subscription(contact_uuid: ContactsUuid, reason: 'bounce' | 'spam'): Promise<void> {
	try {
		...
		const contact = contacts[0] as { uuid: ContactsUuid; email: string; subscriptions: ContactSubscriptions[] | null }
