...
export abstract class Subscribe_Contact_Controller {
    static async http(req: Request, res: Response) {
        ...
        if (!response_ok) {
            ...
            Subscribe_Contact_Controller.create_failure_event(req, response_error).catch(console.error)
        ...
        Welcome_Email.send_log_update(
            API.EVENTS.ORIGINS.SUBSCRIBE_CONTACTS,
            response.contacts[0],
            addresses?.[0],
        ).catch(console.error)
    }
    ...
    static async create_success_event(
        ...
        const success_subscribe_event: EventsInitializer = {
            action: API.EVENTS.ACTIONS.SUBSCRIBE_CONTACTS,
