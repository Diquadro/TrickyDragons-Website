...
export async function send_welcome_email_vip_controller(req: Request, res: Response) {
    ...
    const contact = await verify_contact_eligibility_for_welcome_email(
        contact_email,
        EMAIL_TEMPLATES.WELCOME_VIP_1,
    )

    // Send email using contact's first_name from database
    await send_welcome_vip_1_email(contact_email, contact.first_name || undefined)
    ...
    const response: Welcome_Email_Response = {
        success: true,
        message: 'VIP welcome email sent successfully',
    }
