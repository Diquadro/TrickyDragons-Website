...
/**
 * One-off script to send the "last chance, extended deadline" follow-up email (launch_all_4)
 * to everyone - both the former non-vip (status = lead) and vip (status = prospect) segments
 * get the same email, same as launch_all_1/2/3. Contacts with status = customer are excluded.
 *
 ...
 *
 * Send order: contacts who opened launch_all_3 first (same previous email for both segments),
 * then everyone else, newest subscriber first.
 *
 * Safe to re-run: eligibility excludes contacts who already have the template in `sent_emails`.
 *
 * Usage:
 *   npx tsx src/server/scripts/send_launch_all_4_email.ts [--dry-run] [--delay-ms=12000]
 */
...
async function main() {
    ...
    const { send_launch_all_4_email } = await import('@shared/templates/emails/launch_all_4/launch_all_4')
    ...
    try {
        ...
        const non_vip_contacts = await get_segment_contacts(
            sql,
            ContactStatus.lead,
            EMAIL_TEMPLATES.LAUNCH_ALL_4,
            EMAIL_TEMPLATES.LAUNCH_ALL_3,
            true,
        )
        ...
        console.log(
            `📧 Found ${contacts.length} contacts eligible for ${EMAIL_TEMPLATES.LAUNCH_ALL_4} (non_vip: ${non_vip_contacts.length}, vip: ${vip_contacts.length}, delay: ${delay_ms}ms)`,
        )
        ...
        for (const contact of contacts) {
            try {
                await send_with_retry(() => send_launch_all_4_email(contact.email), contact.email)
                await mark_email_as_sent(contact.uuid, EMAIL_TEMPLATES.LAUNCH_ALL_4)

                sent++
                console.log(
                    `✅ Sent ${EMAIL_TEMPLATES.LAUNCH_ALL_4} to ${contact.email} (${sent}/${contacts.length})`,
                )
            } catch (error) {
                failed++
                console.error(`❌ Failed to send ${EMAIL_TEMPLATES.LAUNCH_ALL_4} to ${contact.email}:`, error)
            }
