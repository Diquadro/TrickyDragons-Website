...
/**
 * One-off script to send the "we're live" launch email (launch_all_1) to everyone - both the
 * former non-vip (status = lead) and vip (status = prospect) segments get the SAME email this
 * time, unlike the earlier pre-launch sends. Contacts with status = customer are excluded.
 *
 ...
 *
 * Send order: contacts who opened their segment's previous email first (pre_launch_non_vip_2
 * for leads, pre_launch_vip_2 for prospects), then everyone else, newest subscriber first.
 *
 * Safe to re-run: eligibility excludes contacts who already have the template in `sent_emails`.
 *
 * Usage:
 *   npx tsx src/server/scripts/send_launch_all_email.ts [--dry-run] [--delay-ms=12000]
 */
...
async function main() {
    ...
    const { send_launch_all_1_email } = await import('@shared/templates/emails/launch_all_1/launch_all_1')
    ...
    try {
        ...
        console.log(
            `📧 Found ${contacts.length} contacts eligible for ${EMAIL_TEMPLATES.LAUNCH_ALL_1} (non_vip: ${non_vip_contacts.length}, vip: ${vip_contacts.length}, delay: ${delay_ms}ms)`,
        )
        ...
        for (const contact of contacts) {
            try {
                await send_with_retry(() => send_launch_all_1_email(contact.email), contact.email)
                await mark_email_as_sent(contact.uuid, EMAIL_TEMPLATES.LAUNCH_ALL_1)

                sent++
                console.log(`✅ Sent ${EMAIL_TEMPLATES.LAUNCH_ALL_1} to ${contact.email} (${sent}/${contacts.length})`)
            } catch (error) {
                failed++
                console.error(`❌ Failed to send ${EMAIL_TEMPLATES.LAUNCH_ALL_1} to ${contact.email}:`, error)
            }
