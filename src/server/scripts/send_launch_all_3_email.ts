...
/**
 * One-off script to send the "24 hours, $13,193 raised" follow-up email (launch_all_3) to
 * everyone - both the former non-vip (status = lead) and vip (status = prospect) segments get
 * the same email, same as launch_all_1/launch_all_2. Contacts with status = customer are
 * excluded.
 *
 ...
 *
 * Send order: contacts who opened launch_all_2 first (same previous email for both segments),
 ...
 *
 * Usage:
 *   npx tsx src/server/scripts/send_launch_all_3_email.ts [--dry-run] [--delay-ms=12000]
 */
...
async function main() {
    ...
    const { send_launch_all_3_email } = await import('@shared/templates/emails/launch_all_3/launch_all_3')
    ...
    try {
        ...
        console.log(
            `📧 Found ${contacts.length} contacts eligible for ${EMAIL_TEMPLATES.LAUNCH_ALL_3} (non_vip: ${non_vip_contacts.length}, vip: ${vip_contacts.length}, delay: ${delay_ms}ms)`,
        )
        ...
        for (const contact of contacts) {
            try {
                await send_with_retry(() => send_launch_all_3_email(contact.email), contact.email)
                await mark_email_as_sent(contact.uuid, EMAIL_TEMPLATES.LAUNCH_ALL_3)

                sent++
                console.log(
                    `✅ Sent ${EMAIL_TEMPLATES.LAUNCH_ALL_3} to ${contact.email} (${sent}/${contacts.length})`,
                )
            } catch (error) {
                failed++
                console.error(`❌ Failed to send ${EMAIL_TEMPLATES.LAUNCH_ALL_3} to ${contact.email}:`, error)
            }
